...
//smartplay - no other suits this trick
double seed=0;
...
//int pdb[15],ndb[15],edb[15],wdb[15];
int *pdb,*ndb,*edb,*wdb;

int main(){
	int p[16],n[16],e[16],w[16],trick[5]={-1,-1,-1,-1,0},deck[52],pass=3,rnd=0,strat[16];
	...
	while (read!=0) {
		...
		switch (read){
			...
			case 1:
				...
				game(n,e,p,w,strat,trick,rnd,nname,ename,pname,wname,deck,pass,NEWGAME);
				break;
			case 2:
				if(loadgame(n,e,p,w,strat,trick,&rnd,nname,ename,pname,wname,deck,&pass)){
					clrscr();
					printplayers(trick,nname,ename,pname,wname);
					game(n,e,p,w,strat,trick,rnd,nname,ename,pname,wname,deck,pass,LOAD);
				}
				break;
			case 3:
				debuggingoptions();
...
int debuggingoptions(){
	...
	while (read!=0){
		clrscr();
		(colourblind==0)?printf("1: Using colours and letters"):printf("1: Using symbols");
		printf("\n2: Toggle debug - ");(debug==1)?printf("ON"):printf("OFF");
		if(debug){
		printf("\n3: Toggle quick debug - ");(quickdebug==1)?printf("ON"):printf("OFF");
		printf("\n4: Toggle computer only - ");(cplay==1)?printf("ON"):printf("OFF");
		printf("\n5: Toggle queen debug - ");(qdbtoggle==1)?printf("ON"):printf("OFF");
		printf("\n6: Toggle function debug - ");(sdbtoggle==1)?printf("ON"):printf("OFF");
		printf("\n7: Toggle comp turn debug - ");(cdbtoggle==1)?printf("ON"):printf("OFF");
		printf("\n8: Toggle lastplay debug - ");(ldbtoggle==1)?printf("ON"):printf("OFF");
		printf("\n9: Toggle trick debug - ");(tdbtoggle==1)?printf("ON"):printf("OFF");
		}
		printf("\n0: Main menu\n\n> ");
		read=getch()-'0';
		switch (read){
...
void ldb(int numPlayed){if(debug&&ldbtoggle){
	char l=(lastplay==N)?'N':(lastplay==E)?'E':(lastplay==P)?'P':'W';
	char n=(lastplay==N)?'E':(lastplay==E)?'P':(lastplay==P)?'W':'N';
	if(numPlayed==4) n='0';
	gotoxy(38,3);printf("last %c",l);//last
	gotoxy(38,4);printf("next %c",n);//next
	return;
}}

void tdb(int *trick){ if(debug&&tdbtoggle){
	char lp;
	int i;
	gotoxy(1,1);
	for(i=0;i<4;i++){
...
int gameOptions(int *n,int *e,int *p,int *w,int *strat,int *trick,int rnd,char *nname,char *ename,char *pname,char *wname,int *deck,int pass){
	...
	while (input==-1){
		...
		switch (input){
			case 1:
				save( n,e,p,w,strat,trick,rnd,nname,ename,pname,wname,deck,pass);
				return 0;
				break;
			case 2:
				debuggingoptions();
